...
class Encoder(torch.nn.Module):
    ...
    def forward(self, xy_batch):
        ...
        y_batch = xy_batch[:,-1]
        neg_aids = torch.randperm(CFG.n_aid, device = xy_batch.get_device())[:len(xy_batch)]
        ...
        x_em = x_em/x_em_norm
        y_em = y_em/torch.norm(y_em, dim=1, keepdim = True)
        cossim = torch.matmul(x_em, y_em.T) / CFG.temperature
        p = torch.softmax(cossim,dim=1)
        labels = torch.arange(len(p), device = xy_batch.get_device())
        loss = - torch.mean(label_type * torch.log(p[labels, labels]))
        acc = torch.mean((torch.max(p,dim=1)[1] == labels).to(torch.float))
        return loss, acc
...
# python emb_model_v15.py
# python emb_model_v15.py --pretrain at_train --is_skip_train --is_inference_all
# python emb_model_v15.py --pretrain at_test --is_inference_all
